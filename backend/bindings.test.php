...
/**
 * Forum module — TEST DI bindings (KernelHarness).
 *
 * Identical to bindings.php except the 4 repository interfaces bind to
 * InMemory fakes instead of SQL implementations — keeps tests fast and
 * DB-free.
 *
 * Loaded by ModuleRegistry::registerBindings($container, ModuleRegistry::TEST)
 * which prefers bindings.test.php over bindings.php when present.
 */
return function (Container $container): void {
    // ---------------------------------------------------------------------
    // 4× Repository bindings — CORE interfaces → IN-MEMORY fakes (test only)
    // ---------------------------------------------------------------------
    $container->singleton(
